---
name: "비정형 치통"
termSlug: bijeonghyeong-chitong
seoTitle: "비정형 치통이란? — 왜 아프고 무엇을 조심하나"
en: "Atypical Odontalgia"
tier: B
aliases:
  - "비정형 치통"
definition: "비정형 치통은 검사에서 원인이 없는데 치아 부위가 계속 아픈 상태예요. 불필요한 치료로 이어지기 쉬워 주의가 필요합니다."
detail: |
  치아나 그 주변이 아픈데 충치도, 금도, 뿌리 끝 염증도 없습니다. 검사와 X-ray가 모두 정상이에요. 그런데 통증은 실재합니다.
  
  특징이 있습니다. 통증이 하루 종일 비교적 일정하게 이어지고, 찬 것이나 뜨거운 것에 반응하지 않으며, 씹어도 크게 달라지지 않아요. 위치를 정확히 짚기 어렵고 여러 치아에 걸쳐 느껴지기도 합니다.
  
  가장 큰 위험은 불필요한 치료입니다. 아프니까 신경치료를 하고, 그래도 아파서 발치를 하고, 그 옆 치아를 또 치료하는 식으로 이어지는 경우가 보고돼요. 치료해도 통증이 그대로라는 점이 중요한 단서입니다.
  
  신경병증성 통증의 범주로 접근합니다. 원인 치료가 아니라 통증 자체를 조절하는 방향이에요. 약물 치료가 중심이고 신경과나 통증 클리닉과 함께 보기도 합니다. 더 이상 치아를 건드리지 않는 것이 중요합니다.
faqs:
  - question: "신경치료를 했는데 계속 아파요."
    answer: "비정형 치통일 수 있습니다. 치료했는데 통증이 그대로인 것이 중요한 단서예요. 추가 치료를 반복하기 전에 감별이 필요합니다."
  - question: "검사가 정상인데 왜 아플까요?"
    answer: "신경계가 통증 신호를 잘못 유지하는 상태로 설명됩니다. 원인이 치아에 없어도 통증은 실재해요. 접근 방법이 달라집니다."
  - question: "어떻게 구분하나요?"
    answer: "온도 자극에 반응하지 않고, 하루 종일 일정하게 아프고, 위치를 짚기 어렵고, 치료해도 변하지 않는 점이 단서입니다."
  - question: "치료 방법이 있나요?"
    answer: "통증 자체를 조절하는 약물 치료가 중심입니다. 신경과나 통증 클리닉과 함께 보기도 해요. 치아를 더 건드리지 않는 것이 중요합니다."
  - question: "이를 빼면 나아질까요?"
    answer: "오히려 통증이 남거나 심해질 수 있습니다. 발치 후에도 같은 부위가 계속 아픈 사례가 보고돼요. 발치는 신중해야 합니다."
approach:
  - when: "진단에서 가장 중요한 것"
    name: "불필요한 치과 처치 중단"
    en: "Avoid Irreversible Treatment"
    detail: "원인 없는 치아에 신경치료나 발치를 반복하면 통증이 더 심해집니다. 이것을 막는 것이 첫 치료입니다."
  - when: "진단이 내려진 뒤"
    name: "신경병증 통증 약물"
    en: "Neuropathic Pain Medication"
    detail: "저용량 삼환계 항우울제나 가바펜틴 계열을 씁니다. 효과까지 4~8주 걸립니다."
  - when: "약물과 병행"
    name: "인지행동 요법"
    en: "Cognitive Behavioral Therapy"
    detail: "통증에 대한 불안이 통증을 키웁니다. 함께 다룰 때 결과가 좋습니다."
  - when: "조절이 어려울 때"
    name: "통증 클리닉 의뢰"
    en: "Pain Clinic Referral"
    detail: "구강안면통증 전문 진료로 넘깁니다. 없애기보다 줄여 지내는 것을 목표로 합니다."
sources:
  - "https://www.ncbi.nlm.nih.gov/books/NBK554486/"
  - "https://www.aae.org/specialty/wp-content/uploads/sites/2/2017/07/endodonticdiagnosisfall2013.pdf"
updated: "2026-08-16"
relatedTerms:
  - extraction
  - root-canal
  - bijeonghyeong-anmyeontong
  - cavity
relatedPosts:
  []
---
