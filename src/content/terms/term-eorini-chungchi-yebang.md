---
name: "어린이 충치 예방"
termSlug: eorini-chungchi-yebang
seoTitle: "어린이 충치 예방 — 언제부터 무엇을 해야 하나"
en: "Caries Prevention in Children"
tier: B
aliases:
  - "어린이 충치 예방"
definition: "어린이 충치 예방은 불소, 실란트, 식습관 관리가 핵심이에요. 첫 치아가 나오면 바로 시작합니다."
detail: |
  예방은 첫 치아가 나오는 순간부터 시작합니다. 거즈나 작은 칫솔로 닦아 주고, 불소 치약을 쌀알 크기로 쓰기 시작해요. 만 3세 이후에는 완두콩 크기 정도로 늘립니다.
  
  혼자 닦게 두는 시기가 생각보다 늦습니다. 손 조절이 충분해지는 만 8세 무렵까지는 부모님이 마무리를 봐 주시는 게 좋아요. 특히 안쪽 어금니는 아이 혼자서는 잘 닿지 않습니다.
  
  만 6세 무렵 나오는 첫 번째 큰어금니에는 실란트를 권합니다. 씹는 면 홈이 깊어 칫솔모가 바닥까지 닿지 않기 때문이에요. 홈을 미리 메워 두면 그 부위 충치를 크게 줄일 수 있습니다.
  
  식습관도 큰 부분이에요. 양보다 빈도가 중요합니다. 단 것을 조금씩 자주 먹는 게 한 번에 먹는 것보다 나빠요. 입안이 산성으로 유지되는 시간이 길어지기 때문입니다. 물 대신 주스나 이온음료를 계속 마시는 습관도 위험해요.
faqs:
  - question: "불소 치약은 언제부터 쓰나요?"
    answer: "첫 치아가 나오면 시작합니다. 만 3세 미만은 쌀알 크기, 그 이후는 완두콩 크기가 기준이에요. 삼키지 않도록 뱉는 연습을 함께 해 주세요."
  - question: "실란트는 언제 하나요?"
    answer: "만 6세 무렵 첫 번째 큰어금니가 나오면 권합니다. 두 번째 큰어금니는 만 12세 전후예요. 홈이 깊은 유치 어금니에 하기도 합니다."
  - question: "아이 혼자 닦게 해도 되나요?"
    answer: "만 8세 무렵까지는 부모님이 마무리를 봐 주시는 게 좋습니다. 손 조절이 아직 부족해 안쪽 어금니가 잘 안 닦여요. 스스로 하게 하되 마지막에 확인해 주세요."
  - question: "단 걸 아예 못 먹게 해야 하나요?"
    answer: "양보다 빈도가 중요합니다. 조금씩 자주 먹는 게 더 나빠요. 간식 시간을 정해 몰아서 먹고 이후 물로 헹구는 편이 낫습니다."
  - question: "검진은 얼마나 자주 받나요?"
    answer: "대개 6개월 간격을 권합니다. 충치가 잘 생기는 아이는 더 자주 보기도 해요. 첫 방문은 첫 치아가 난 뒤 또는 돌 무렵을 기준으로 잡습니다."
sources:
  - "https://www.aapd.org/research/oral-health-policies--recommendations/"
  - "https://www.ada.org/resources/ada-library/oral-health-topics/fluoride-topical-and-systemic-supplements"
updated: "2026-08-16"
relatedTerms:
  - cavity
  - yebangjeog-hangsaengje
  - yuchi-chungchi
  - root-canal
relatedPosts:
  []
---
